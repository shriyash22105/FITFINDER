...
function VirtualTryOn() {
  ...
  return (
    <div>
      <input type="file" onChange={(e) => setHumanImage(e.target.files[0])} />
      <input type="file" onChange={(e) => setClothImage(e.target.files[0])} />
      <button onClick={handleTryOn} disabled={loading}>
        {loading ? 'Processing...' : 'Try On'}
      </button>
      {result && <img src={result.data.image} alt="Result" />}
    </div>
  );
}
```
...
<template>
  <div class="try-on">
    <input type="file" ref="humanInput" accept="image/*" />
    <input type="file" ref="clothInput" accept="image/*" />
    <button @click="tryOn" :disabled="loading">
      {{ loading ? 'Processing...' : 'Try On' }}
    </button>
    <img v-if="result" :src="result.data.image" alt="Result" />
  </div>
</template>
...
export default {
  ...
  methods: {
    async tryOn() {
      const formData = new FormData();
      formData.append('humanImage', this.$refs.humanInput.files[0]);
      formData.append('clothImage', this.$refs.clothInput.files[0]);
      formData.append('garmentType', 'full_body');
...
- **Human Image**: 512 x 768 pixels (portrait)
- **Cloth Image**: 256 x 384 pixels (portrait)
- **File Size**: < 5MB each

### Expected Response Times
- **Login**: 50-150ms
- **Single Try-On (Local)**: 500-2000ms
- **Combo Try-On (Local)**: 800-3000ms
- **With Miragic API**: 2-60 seconds (depends on queue)
...
```json
{
  "success": false,
  "error": "Processing failed",
  "message": "Unable to process images: File not readable"
}
```
...
*To be implemented with HTTP 429 Too Many Requests*

**Current Limits (Recommended):**
- Authentication: 5 requests per minute per IP
- Try-On: 10 requests per minute per user
